...
def validate_runtime_args(args: argparse.Namespace) -> TrackFocus | None:
    ...
    if not args.force_archetype:
        raise ArgumentValidationError("--force-track-focus requires --force-archetype.")
    ...
    if not supports_track_focus(forced_archetype):
        supported = ", ".join(
            ...
            if supports_track_focus(archetype)
        )
        raise ArgumentValidationError(
...
def _load_station_runtime(
    ...
    if channel.azuracast_station_id is not None:
        ...
        LOGGER.info("[station] Using configured AzuraCast station ID %s.", station_id)
    else:
...
def _fetch_playback_context(
    ...
    *,
    ...
    minimum_lead_seconds: int = LEAD_TIME_SECONDS,
    ...
    if current_remaining is None:
        ...
        return None
    if current_remaining < minimum_lead_seconds:
        LOGGER.info(
...
def _fetch_queue_context(
    ...
    schedule_reference_ts = timestamp + max(0, playback.current_remaining)
...
def _resolve_effective_forced_archetype(
    ...
    forced_archetype = Archetype(args.force_archetype) if args.force_archetype else None
...
def _build_generation_context(
    ...
    hook = choose_hook(selected_archetype, state, rng, runtime.channel.archetype_policy)
    ...
    if selected_archetype == Archetype.NEWS:
        LOGGER.info(
            "[news] Archetype selected; topic rolls will be logged in generation."
        )
...
def _publish_segment(
    ...
    rng: random.Random,
    before_inject: (
        Callable[[], tuple[PlaybackContext, QueueContext] | None] | None
    ) = None,
) -> PublishResult:
    ...
    if before_inject is not None:
        refreshed = before_inject()
        if refreshed is None:
            raise BlockIntroDeferred("Block intro boundary changed during upload")
        playback, queue_context = refreshed

    if before_inject is not None:
        # A timeout can mean Liquidsoap accepted the request but its response
        # was lost. Prepared intros claim their attempt durably before sending;
        # retrying this non-idempotent command could play the intro twice.
        telnet_response = runtime.client.send_telnet_command(
            runtime.station_id, telnet_command
        )
    else:
        telnet_response = run_with_retries(
            "Queue media via telnet",
            lambda: runtime.client.send_telnet_command(
                runtime.station_id, telnet_command
            ),
        )
...
class BlockIntroDeferred(RuntimeError):
    """The prepared intro no longer has a safe next-boundary insertion slot."""


def _publish_prepared_intro(
    *,
    args: argparse.Namespace,
    runtime: StationRuntime,
    prepared: PreparedBlockIntro,
    state,
    rng: random.Random,
    deps: HostRuntimeDependencies,
) -> HostCycleResult:
    def refresh() -> tuple[PlaybackContext, QueueContext] | None:
        playback = _fetch_playback_context(
            args,
            runtime.client,
            state,
            update_seen_state=False,
            minimum_lead_seconds=BLOCK_INTRO_INSERT_LEAD_SECONDS,
        )
        if playback is None:
            return None
        queue = _fetch_queue_context(
            args,
            runtime.client,
            playback,
    ...
    def claim_submission() -> tuple[PlaybackContext, QueueContext] | None:
        refreshed = refresh()
        if refreshed is not None:
            state.pending_block_intro["submission_attempted"] = True
            _, state_path, _ = deps.station_state_paths(runtime.channel.key)
    ...
    if refreshed is None:
        return HostCycleResult(
            status="skipped",
            reason="waiting for block intro boundary",
            station=args.station,
            channel=runtime.channel.key,
        )
    playback, queue = refreshed
    if args.dry_run:
        return HostCycleResult(
            status="generated",
            reason="prepared block intro ready (dry run)",
            station=args.station,
            channel=runtime.channel.key,
    ...
    try:
        published = deps.publish_segment(
            args=args,
            runtime=runtime,
            playback=playback,
            queue_context=queue,
            ...
            segment_title=prepared.title,
            news_segment=None,
            script_text=prepared.assets.story_text,
            assets=prepared.assets,
            state=state,
            rng=rng,
            before_inject=claim_submission,
        )
    except BlockIntroDeferred:
        LOGGER.info(
            "[schedule] Deferring prepared intro after queue changed during upload."
        )
        return HostCycleResult(
            status="skipped",
            reason="block intro boundary changed during upload",
            station=args.station,
            channel=runtime.channel.key,
        )
    state.pending_block_intro = None
    return HostCycleResult(
        status="published",
        reason=None,
        station=args.station,
        ...
        next_track=queue.next_track,
        selected_archetype=Archetype.BLOCK_INTRO,
        used_archetype=Archetype.BLOCK_INTRO,
        segment_title=prepared.title,
        assets=prepared.assets,
        queued_request_id=published.queued_request_id,
        expected_play_at_utc=published.expected_play_at_utc,
    )
...
class HostRuntimeDependencies:
    ...
    generate_script: Callable[..., tuple[str, GeneratedSegmentMetadata, Archetype]] = (
        generate_archetype_script
    )
    ...
    def run_cycle(self, request: HostCycleRequest) -> HostCycleResult:
        ...
        try:
            ...
            playback = _fetch_playback_context(
                ...
                minimum_lead_seconds=(
                    0 if not args.force_archetype else LEAD_TIME_SECONDS
                ),
            )
            if playback is None:
                return HostCycleResult(
            ...
            if not args.force_archetype:
                ...
                if (
                    ...
                    and prepared.awaiting_boundary(
                        runtime.schedule_state,
                        deps.now(),
                        playback.current_track,
                        playback.current_remaining,
                    ...
                    return HostCycleResult(
                        status="skipped",
                        reason="waiting for block intro queue timestamps",
                        station=args.station,
                        channel=channel.key,
                    )
                if prepared is not None and intro_plan is not None:
                    if (
                        state.pending_block_intro.get("submission_attempted")
                        and prepared.context.block_key == intro_plan.context.block_key
                    ):
                        LOGGER.warning(
                            "[schedule] Intro submission was already attempted; not sending a duplicate."
                        )
                        return HostCycleResult(
                            status="skipped",
                            reason="block intro submission already attempted",
                            station=args.station,
                            channel=channel.key,
                        )
                    if (
                        prepared.matches(intro_plan)
                        and prepared.assets.audio_path.is_file()
                    ):
                        LOGGER.info(
                            "[schedule] Reusing prepared intro for '%s'; target=%s - %s; queue position=%s.",
                            intro_plan.context.section_label,
                            intro_plan.target.artist,
                            intro_plan.target.title,
                            intro_plan.target_index + 1,
                        )
                        return _publish_prepared_intro(
                            args=args,
                            runtime=runtime,
                            prepared=prepared,
                            state=state,
                ...
                if state.pending_block_intro and not args.dry_run:
                    LOGGER.info(
                        "[schedule] Discarding pending intro: target/context changed or boundary passed."
                    )
                    state.pending_block_intro = None
                if intro_plan is not None:
                    # Generate as if we were already at the target boundary.
                    # Both the preceding song and the announced song must be
                    # those around that future break, not today's queue head.
                    playback = replace(
                        playback,
                        current_track=intro_plan.predecessor,
                        current_key=track_key(
                            intro_plan.predecessor.artist, intro_plan.predecessor.title
                        ),
                        current_remaining=max(
                            0, int(intro_plan.expected_play_at - deps.now())
                        ),
                        recent_tracks=(),
                    )
                    queue_context = QueueContext(
                        upcoming_tracks=intro_plan.upcoming_tracks,
                        next_track=intro_plan.target,
                        schedule_context=intro_plan.context,
                        schedule_reference_ts=intro_plan.expected_play_at,
                    )
                    LOGGER.info(
                        "[schedule] Preparing intro for '%s' before %s - %s (queue position %s, expected %s).",
                        intro_plan.context.section_label,
                        intro_plan.target.artist,
                        intro_plan.target.title,
                        intro_plan.target_index + 1,
                        iso_utc(intro_plan.expected_play_at),
                    )
                elif queue_context.schedule_context is not None:
                    # Start-window context alone must never cause a late intro
                    # after the first boundary has already passed.
                    if queue_context.schedule_context.mention_intent == "start":
                        queue_context = replace(
                            queue_context,
                            schedule_context=replace(
                                queue_context.schedule_context, mention_intent=None
                            ),
                        )
                if (
                    intro_plan is None
                    and playback.current_remaining < LEAD_TIME_SECONDS
                ):
                    return HostCycleResult(
                        status="skipped",
                        reason="insufficient generation lead time",
                        station=args.station,
                        channel=channel.key,
                    )
            ...
            if intro_plan is not None and not args.dry_run:
                if archetype_used != Archetype.BLOCK_INTRO:
                    return HostCycleResult(
                        status="skipped",
                        reason="block intro generation fell back to another archetype",
                        station=args.station,
                        channel=channel.key,
                    )
                prepared = PreparedBlockIntro(
                    context=intro_plan.context,
                    target=intro_plan.target,
                    predecessor=intro_plan.predecessor,
                    assets=assets,
                    title=segment_title,
                    hook=generation_context.hook,
                    angle=generation_context.angle,
                )
                state.pending_block_intro = prepared.to_dict()
                deps.save_state(state_path, state)
                # Generation may have taken long enough for the target to move
                # to the front. Re-read playback before attempting insertion.
                return _publish_prepared_intro(
                    args=args,
                    runtime=runtime,
                    prepared=prepared,
