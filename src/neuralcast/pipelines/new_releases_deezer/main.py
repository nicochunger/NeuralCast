"""Update station's New Releases playlist with latest tracks via Deezer."""
...
_EXCLUDED_PLAYLIST_FILENAMES = {
    "new releases.csv",
    "new releases deezer.csv",
}
...
def _save_metadata_entries(
    ...
    if dry_run:
        log_info("Dry run: not writing Deezer metadata JSON")
        return
...
def load_existing_new_releases(playlists_dir: Path) -> list[ArtistRelease]:
    ...
    if not path.exists():
        log_debug("New Releases Deezer.csv not found; starting from empty state")
        return []
    ...
    for _, row in df.iterrows():
        ...
        if release_raw:
            try:
                ...
                log_debug(f"Invalid Deezer ReleaseDate '{release_raw}' for {artist} - {title}")
...
def save_new_releases(playlists_dir: Path, releases: list[ArtistRelease], dry_run: bool) -> None:
    ...
    if not releases:
        log_info("No Deezer new releases to write.")
        print("No Deezer new releases to write.", file=sys.stderr)
        return
    ...
    if dry_run:
        log_info("Dry run: not writing Deezer CSV")
        print("Dry run: not writing Deezer CSV", file=sys.stderr)
    ...
    log_success(f"Wrote {len(df_csv)} Deezer tracks → {output_path}")
    print(f"Wrote {len(df_csv)} tracks to {output_path}", flush=True)


def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Refresh the Deezer-based New Releases test playlist for a station."
    )
    parser.add_argument(
        "-s",
        "--station",
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Collect and display results without writing Deezer output files",
    )
    parser.add_argument(
        "--per-artist",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Minimum Deezer rank to keep (default: 0)",
    )
    parser.add_argument(
        "--prefer-singles",
        action="store_true",
...
def main() -> None:
    ...
    if outdated_existing:
        log_info(
            f"Skipped {len(outdated_existing)} aged Deezer test tracks; no migration is performed in this standalone pipeline"
        )

    if final_releases:
        log_info(f"Collected {len(final_releases)} recent Deezer tracks")
        print(f"Collected {len(final_releases)} recent Deezer tracks", flush=True)
    else:
        log_info("No Deezer releases found within the window")
        print("No Deezer releases found within the window", flush=True)
