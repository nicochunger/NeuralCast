...
Station data still lives alongside the code in station folders (currently `NeuralCast/` and `NeuralForge/`). Each station keeps `playlists/` (CSV definitions), `songs/` (MP3 catalog mirrored per playlist), `metadata/` (Spotify/Deezer New Releases caches + orchestrator/schedule state/logs), and `tts_snippets/` for generated/scripted drops. Generated reports such as `duplicate_analysis.log` land in the station folder.
