...
- `zip -r deployment/deploy_host_orchestrator.zip src vps_requirements.txt -x "*/__pycache__/*" "*.pyc" "src/neuralcast/assets/stories/snippets/*"` rebuilds the VPS host orchestrator bundle after code/prompt changes.
...
When asked to redeploy the host orchestrator package to the VPS, always perform all three steps in order:
